...
- The optional accounts file (`ACCOUNTS_FILE`) is an operator-supplied credential store: mode 600 (no group/world permission bits) is enforced at startup, the file is capped at 64 KiB and 1–16 accounts, the default `accounts.json` filename is gitignored, and its contents are never logged; at request time only per-account SHA-256 token digests are compared.
